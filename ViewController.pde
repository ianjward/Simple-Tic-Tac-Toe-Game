...
public void createGUI(){
 ...
 //caller is 1 if the move being checked was made by the user. O if it was made by AI.
private void checkMove(MyImageButton square, int caller){
  boolean setAValidSquare = false;
  if(square.isAvailable){
    ...
    movesCompleted++;
    checkForVictory();
    setAValidSquare = true;
  }
  if(movesCompleted == 9 & !gameOver){
     gameOver = true;
     Menu.setLocalColorScheme(GCScheme.GOLD_SCHEME);
     popUp("The game was a draw, click restart to play again!");
   }else if(userWon){
     Menu.setLocalColorScheme(GCScheme.GREEN_SCHEME);
     popUp("You won, click restart to play again!"); 
   }else if(aiWon){
       Menu.setLocalColorScheme(GCScheme.RED_SCHEME);
     popUp("You lost, click restart to play again!"); 
   }else if(caller == 1 && setAValidSquare){
     aiPick();
   }
}
...
public void aiPick(){
  ...
  while(allButtons[randomSquare].isAvailable == false){ //make sure ai does't choose an unavailable square
...
public void imgButton1_click1(MyImageButton source, GEvent event) { 
  checkMove(imgButton1, 1);
} 

public void imgButton2_click1(MyImageButton source, GEvent event) { 
   checkMove(imgButton2, 1);
} 

public void imgButton3_click1(MyImageButton source, GEvent event) { 
   checkMove(imgButton3, 1);
}

public void imgButton4_click1(MyImageButton source, GEvent event) { 
   checkMove(imgButton4, 1);
} 

public void imgButton5_click1(MyImageButton source, GEvent event) { 
   checkMove(imgButton5, 1);
} 

public void imgButton6_click1(MyImageButton source, GEvent event) { 
   checkMove(imgButton6, 1);
}

public void imgButton7_click1(MyImageButton source, GEvent event) { 
  checkMove(imgButton7, 1);
} 

public void imgButton8_click1(MyImageButton source, GEvent event) { 
  checkMove(imgButton8, 1);
} 

public void imgButton9_click1(MyImageButton source, GEvent event) { 
